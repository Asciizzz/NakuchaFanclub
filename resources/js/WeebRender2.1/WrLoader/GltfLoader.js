...
async function parseGLB(url) {
	...
	if (dv.getUint32(0, true) !== 0x46546c67) throw new Error(`[WrGltfLoader] not a GLB: ${url}`);
...
function buildSceneData(gltf, modelName, meshIndexToId, skinToSkeletonId) {
	...
	const rootId = pushNode(modelName || "Model_Root", null, {
		Transform: { local: Alm.Mat4.makeIdentity(), world: Alm.Mat4.makeIdentity() },
	});
	...
	if ((gltf.skins?.length || 0) > 0) {
		for (let si = 0; si < gltf.skins.length; si++) {
			...
			const holderId = pushNode(nameOr(gltf.skins[si].name, "Skeleton", si), null, {
				Transform: { local: Alm.Mat4.makeIdentity(), world: Alm.Mat4.makeIdentity() },
	...
	const gltfParent = new Int32Array(gltf.nodes?.length || 0).fill(-1);
	for (let gi = 0; gi < (gltf.nodes?.length || 0); gi++) {
		const gn = gltf.nodes[gi];
		if (!gn.children) continue;
		for (const cgi of gn.children) gltfParent[cgi] = gi;
	}
	...
	// glTF hierarchy with skeleton insertion.
	for (let gi = 0; gi < (gltf.nodes?.length || 0); gi++) {
		if (isJoint[gi]) continue;
		const sceneNodeId = gltfNodeToSceneNodeId[gi];
		if (!sceneNodeId) continue;

		const gn = gltf.nodes[gi];
		const parentGi = gltfParent[gi];
		const parentSceneId = (parentGi >= 0 && !isJoint[parentGi]) ? gltfNodeToSceneNodeId[parentGi] : null;

		if (gn.mesh != null && gn.skin != null) {
			const holderId = skinToHolderNodeId[gn.skin] ?? null;
			if (holderId) {
				const holderIndex = indexById.get(holderId);
				const holder = holderIndex != null ? nodes[holderIndex] : null;
				if (holder && holder.parent == null) setParent(holderId, parentSceneId ?? rootId);
				setParent(sceneNodeId, holderId);
				continue;
			}
		}

		if (parentSceneId) setParent(sceneNodeId, parentSceneId);
	}

	// Unparented nodes attach under scene root.
	for (const n of nodes) {
		if (n.id === rootId) continue;
		if (n.parent != null) continue;
		setParent(n.id, rootId);
	}
