...
export class WrSkeleton {
	constructor(raw = {}) {
		...
		this.name = source.name ?? "skeleton";
	...
	rebuildBoneCache() {
		const source = Array.isArray(this.bones) ? this.bones : [];
		const names = new Array(source.length);
	...
	buildPalette(localPoses = [], maxBones = 128) {
		...
		for (let i = 0; i < cap; i += 1) out.set(Azm.Mat4.IDENTITY, i * 16);

		const poses = this.ensurePoseCapacity(localPoses, source.length);
		const global = new Array(source.length);
		const count = Math.min(source.length, cap);
