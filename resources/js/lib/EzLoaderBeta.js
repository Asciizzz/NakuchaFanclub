 ...
 *       // Back-compat single channel alias (POSITION):
 *       morphChannel:        "u_morphPosTex",
 *       // Preferred explicit channels:
 *       morphPositionChannel:"u_morphPosTex",
 *       morphNormalChannel:  "u_morphNrmTex",
 *       morphTangentChannel: "u_morphTanTex",
...
(function () {
    ...
    async function load(url, opts = {}) {
        const {
            ...
            shaderKey     = null,
            morphChannel  = "u_morphPosTex",
            morphPositionChannel,
            morphNormalChannel = null,
        ...
        const morphChanMap = [
            { gltfAttr: "POSITION", outName: morphPosCh },
            { gltfAttr: "NORMAL",   outName: morphNormalChannel },
            { gltfAttr: "TANGENT",  outName: morphTangentChannel },
        ].filter(e => typeof e.outName === "string" && e.outName.length > 0);
        ...
        for (const { meshIdx, transform } of collectMeshNodes(gltf)) {
            ...
            for (const prim of mesh.primitives) {
                ...
                if (Array.isArray(prim.targets) && prim.targets.length > 0) {
                    const ch = {};
                    for (const m of morphChanMap) {
                        const hasAny = prim.targets.some(tgt => tgt[m.gltfAttr] != null);
                        if (!hasAny) continue;
                        ch[m.outName] = prim.targets.map(tgt =>
                            tgt[m.gltfAttr] != null ? readAccessor(tgt[m.gltfAttr]) : new Float32Array(vcount * 3)
                        );
                    }

                    morphTargets = Object.keys(ch).length > 0 ? ch : null;

                    if (prim.targets.length > logicalMorphCount) logicalMorphCount = prim.targets.length;
                    if (!morphTargetNames && meshTargetNames) morphTargetNames = meshTargetNames.slice();
                    primMorphInfo.push({ offset: morphRunOffset, count: prim.targets.length });
                    morphRunOffset += prim.targets.length;
